  ...
  return [
    ...
    },
    {
      ...
      content: [
        ...
        '1. The knowledge context below has already been retrieved. It satisfies the documentation lookup requirement. Answer directly when it contains enough evidence; tools are optional, not a checklist.',
        ...
        '3. For other config edits, use target_file only if the target file is unclear and the client is known. Use make_patch for scalar path/value edits; it cannot generate lists such as schema_list. Write list-based YAML directly from documented evidence. If the client or existing schema list is needed to preserve user settings, ask one focused clarification instead of repeatedly searching.',
        ...
        '5. Use search_docs only for a specific missing documentation detail, at most twice per user request. Never repeat the same query or keep rewording it when results add no new evidence. After that, answer from the evidence or explain what information is missing.',
        '6. There are at most six model turns, with the final turn reserved for an answer and no tools. Once enough evidence is available, stop using tools and respond immediately.',
