...
export function WelcomeScreen({ onPrompt }: { onPrompt: (prompt: string) => void }) {
  return <div className="welcome-screen">
    ...
    <div className="guide-grid">
      <article><b>⌕</b><span><strong>上传文件诊断</strong>上传 YAML 或 Lua，快速定位常见配置问题。</span></article>
      <article><b>▢</b><span><strong>直接发起提问</strong>描述目标效果，生成可直接使用的配置 Patch。</span></article>
    </div>
    ...
    <div className="suggestion-grid">{suggestions.map(([label, prompt]) => <button type="button" key={label} onClick={() => onPrompt(prompt)}><b>☆ {label}</b><span>{prompt}</span></button>)}</div>
  </div>;
}
